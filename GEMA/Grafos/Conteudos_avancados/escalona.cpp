...
priority_queue<int, vector<int>, greater<int>> fila;        //armazena os vértices que podem ser visitados (não dependem de outros vértices)
...
int main() {
    ...
    if (vistos < n || n==0) {   //caso haja ciclos no grafico
        cout << "*";
    } else {
        for (int i : ordenados) {
            cout << i << "\n";
