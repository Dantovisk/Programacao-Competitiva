...
int query(int no, int l, int r, int val, int id){
    ...
    if(mid < id || val > tree[no*2]) return query(no*2 + 1, mid+1, r, val, id);
    if(l>=id) return query(no*2, l, mid, val, id);

    int a = query(no*2 + 1, mid+1, r, val, id);
    int b = query(no*2, l, mid, val, id);
