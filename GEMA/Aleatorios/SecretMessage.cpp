...
namespace seg {
	...
	ll query(int a = 0, int b = n-1, int p=1, int l=0, int r=n-1) {
		prop(p, l, r);
...
void solve(){
    ...
    for(int i = 0; i<n-1; i++){
        auto [w, a, b] = e[i];
        sum += w;
        hld::g[a].push_back({b, w});
        hld::g[b].push_back({a, w});
    }


    //verifica se ja é solução
    queue<int> q;
    vi vis(n+1, 0);
    int cont = 0;
    q.push(1); vis[1] = 1;
    ...
    for(int i = n-1; i<m; i++){
        ...
        hld::update_path(a, b, uai);

        ll aux = seg::query();
        // cout<<"aux: "<<aux<<"\n";
        if(aux > 0) res = min(res, sum + w - aux);

        hld::update_path(a, b, -uai);
    }
    if(m-n >= 1 && n >= 3){
        ll aux = sum - get<0>(e[n-2]) - get<0>(e[n-3]) + get<0>(e[n-1]) + get<0>(e[n]);
        res = min(res, ll(aux));
    }
